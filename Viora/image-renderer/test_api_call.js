...
const foodData = {
    ...
    fiber: 1,
    score: 3,
    alerts: ["⚠️ Baixa quantidade de vegetais", "⚠️ Alto teor de sódio"],
    sodium: 150,
    is_food: true,
    protein: 8,
    calories: 120,
    ...
    meal_type: "almoço",
    coach_humor: "Isso é um prato ou uma obra de arte abstrata? Vamos melhorar essa apresentação, recruta!",
    satiety_index: "baixo",
    ...
    image_url: "https://mnhgpnqkwuqzpvfrwftp.supabase.co/storage/v1/object/public/consultas/91a1667f-2b76-41e8-a688-fc1b4cc6f03d/food_1780609819398.jpeg"
};
...
async function renderCard(filename, postData) {
    try {
        console.log(`Calling http://localhost:3001/api/render for ${filename}...`);
        ...
        console.error(`Error rendering ${filename}:`, e);
    }
}

async function main() {
    // 1. Test Client HTML (Passes html in body)
    console.log("=== Testing Client-Side HTML template ===");
    const clientHtml = buildHtmlTemplate(foodData, "militar");
    await renderCard("test_card_client.png", {
        html: clientHtml,
        width: 720,
        height: 1280
    });

    // 2. Test Server Fallback HTML (Passes data, server builds html)
    console.log("\n=== Testing Server-Side Fallback HTML template ===");
    await renderCard("test_card_server.png", {
        data: {
            ...foodData,
            coach_personality: "militar"
        }
    });
}
