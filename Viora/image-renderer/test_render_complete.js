...
// Raw text from user request (simulating the bot's parsed input)
const payload = {
  ...
  image_url: null, // Simulate no photo uploaded or placeholder
  coach_personality: "gordon_ramsay"
};

// HTML Template builder function
function buildHtml(data) {
    const food_name = data.food_name || 'Prato Identificado';
    const calories = Math.round(data.calories || 0);
    const protein = Math.round(data.protein || 0);
    const carbs = Math.round(data.carbs || 0);
    const fat = Math.round(data.fat || 0);
    const fiber = Math.round(data.fiber || 0);
    const sodium = Math.round(data.sodium || 0);
    const score = data.score || 0;
    const meal_type = data.meal_type || '';
    const cuisine_origin = data.cuisine_origin || '';
    const estimated_weight_g = data.estimated_weight_g || 0;
    const satiety_index = data.satiety_index || 'médio';
    const glycemic_index = data.glycemic_index || 'médio';
    const energy_duration_minutes = data.energy_duration_minutes || 0;
    const goal_fit = data.goal_fit || { emagrecer: 0, ganhar_massa: 0, manter: 0 };
    const alerts = data.alerts || [];
    const swap_suggestions = data.swap_suggestions || [];
    const coach_humor = data.coach_humor || '';
    const image_url = data.image_url || null;
    const coach_personality = data.coach_personality || 'gordon_ramsay';
...
async function main() {
    try {
        console.log("Generating HTML template with mock payload...");
        const finalHtml = buildHtml(payload);

        console.log("Posting HTML to https://puppeteer.jeanspagolla.com.br/api/render ...");
        const response = await fetch("https://puppeteer.jeanspagolla.com.br/api/render", {
            method: "POST",
            headers: {
        ...
        console.log("Response received. Reading buffer...");
        ...
        console.error("Error fetching/saving test card image:", e);
    }
}
